...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        z = self.conv3(x) # 操作通常被称为“shortcut connection”或“identity mapping”。它的目的是确保残差块的输入x和输出能够相加
        return F.relu(y+z)
        
class ImgEncoder(nn.Module):
    def __init__(self):
        super().__init__()
        self.res_block1 = ResidualBlock(in_channels=1, out_channels=16, stride=2) #(b, 16, 14, 14)
        self.res_block2 = ResidualBlock(in_channels=16, out_channels=4, stride=2) #(b, 4, 7, 7)
        self.res_block3 = ResidualBlock(in_channels=4, out_channels=1, stride=2) #(b, 1,4, 4)
    ...
    def forward(self, x):
        ...
        x = self.res_block2(x)
        x = self.res_block3(x)
        x = self.wi(x.view(x.size(0), -1))  # 如果用Flatten: x = x.flatten(1)  # 从第二个维度开始展平
        x = self.ln(x)
        return x
